...
After correctly configuring the database, there is a list of the modules that can be installed by default with the farmOS installation. We will leave the ones that are already selected and also add the ones related to sensors, as we are going to be using them for sure.


However, these modules can be installed later on, using drush. We will see how to do this a bit further down, because it is tremendously useful when managing Drupal sites from the developer's point of view, as it is our particular case.
...
\textbf{How to install and enable a module using Drush}
...
The reader might have noticed that the sensor type is listener. The farm sensor listener is a submodule that extends the Sensor asset type by providing integration with external devices. To connect to specific types of sensors, additional modules can be built. For more information in relation to farm sensors, refer to the \href{https://farmos.org/guide/assets/sensors/}{documentation}.
...
'{ "timestamp": 1633626889, "value": 76.5 }'
http://localhost/farm/sensor/listener/4840abd4e88034979a0a8ae4ffdcd5f9?private_key=65fcb0fae0b5813cf4449637de11af9a
\end{verbatim}
