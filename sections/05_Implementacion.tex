...
Before we step in, it catchs our eye that there are two well distingued functions: \verb|void setup()| and \verb|void loop()|. There two functions are \textbf{mandatory} in Arduino. If we try to execute code that lacks any of them, we'll get an error. See an example below.
